...
b1 = tsds.load_ozone()
df = b1.mPastData
df[b1.mTimeVar + '_Weekly'] = pd.date_range('2000-1-1', periods=df.shape[0], freq='W')
df[b1.mTimeVar + '_Daily'] = pd.date_range('2000-1-1', periods=df.shape[0], freq='D')
df[b1.mTimeVar + '_Hourly'] = pd.date_range('2000-1-1', periods=df.shape[0], freq='h')
df[b1.mTimeVar + '_PerMinute'] = pd.date_range('2000-1-1', periods=df.shape[0], freq='1min')
df[b1.mTimeVar + '_PerSecond'] = pd.date_range('2000-1-1', periods=df.shape[0], freq='s')
...
for timevar in [b1.mTimeVar + '_Weekly', b1.mTimeVar + '_Daily',
                b1.mTimeVar + '_Hourly', b1.mTimeVar + '_PerMinute',
                b1.mTimeVar + '_PerSecond']:
